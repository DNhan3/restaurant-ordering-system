          ...
          <div>
            ...
            <ul className="space-y-2">
              {['Phở & Bún', 'Cơm', 'Bánh Mì', 'Cuốn & Khai Vị', 'Đồ Uống', 'Combo'].map((item) => (
                <li key={item}>
                  <Link to={`/menu?category=${item === 'Phở & Bún' ? 'pho' : item === 'Cơm' ? 'com' : item === 'Bánh Mì' ? 'banhmi' : item === 'Cuốn & Khai Vị' ? 'cuon' : item === 'Đồ Uống' ? 'douong' : 'combo'}`} className="text-brown-300 hover:text-primary transition-colors text-sm">
                    {item}
                  </Link>
                </li>
              ))}
            ...
            <ul className="space-y-3">
              <li className="flex items-start gap-3">
                ...
                <span className="text-brown-300 text-sm">
                  36 HCMIU, Quận 1, TP. Hồ Chí Minh
                </span>
              </li>
